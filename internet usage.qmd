...
Broadband access by country at 30 megabits per second (Mbps):
...
Broadband access by country at 100 megabits per second (Mbps):
...
ggplot(rearranged_country, aes(x = fct_rev(fct_inorder(Country)), y = Value, fill = as.factor(Year))) +
  ...
  labs(title = "Comparison of Countries in 2022 and 2012",
       x = "Country",
       y = "% Households with Internet Acces") +
...
shop_pattern <- co_house_access |>
  filter(REG_ID == "NL") |>
  inner_join(dd, by = c("REG_ID", "Year"))
```
